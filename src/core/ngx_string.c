 ...
 *   {
 *        strncpy(dst, src, n - 1);
 *        dst[n - 1] = '\0'; //至少对dst[n-1]赋值一次
 *        return dst;
 *   }
 */
/* strlcpy的升级版：最多对dst[n-1]赋值一次 */
u_char *
...
ngx_atoi(u_char *line, size_t n)
{
    ...
    for (value = 0; n--; line++) {
        ...
        /* 等价于(value > cutoff) || (value == cutoff && *line - '0' > cutlim) 
		 * 即前len - 1位不能超过cutoff，且前len - 1位等于cutoff时最后一位不能超过cutlim
		 * 确保不超过NGX_MAX_INT_T_VALUE
		 * */
