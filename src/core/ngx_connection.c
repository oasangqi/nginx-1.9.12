...
ngx_get_connection(ngx_socket_t s, ngx_log_t *log)
{
	...
	/* 为了保持连接与读、写事件的对应关系不变，所以临时保存读、写事件 */
    rev = c->read;
    wev = c->write;
	...
	/* 每取出一个连接，都将对应事件的instance位取反，标记事件是否过期 */
...
/* reusable为1，将c加入reusable_connections_queue队列，
 * reusable为0，则将其从队列移除，表示不能被强行关闭 */
void
ngx_reusable_connection(ngx_connection_t *c, ngx_uint_t reusable)
{
    ...
    if (reusable) {
		...
		/* 插入队列首部，ngx_drain_connections从队尾开始强制断开连接 */
