...
/* 模块的基本接口 */
struct ngx_module_s {
	/* 对于一类模块而言，ctx_index标示当前模块在这类模块中的序号.
	 * 该成员由管理这类模块的一个nginx核心模块设置的，
	 * 参照ngx_count_modules 
	 * */
    ngx_uint_t            ctx_index;
    ngx_uint_t            index; /* 在ngx_modules数组中的位置 */
    ...
    ngx_uint_t            version; /* 模块版本，暂定为1 */
    const char           *signature;

	/*
	 * 模块上下文，每个模块有不同模块上下文,每个模块都有自己的特性，而ctx会指向特定类型模块的公共接口。
	每一类模块的ctx执行的结构可能都不一样，比如，在event模块中，ctx需要指向ngx_event_module_t结构体。
	*/
    void                 *ctx;
    ngx_command_t        *commands;
    ngx_uint_t            type; /* 模块类型，NGX_CORE_MODULE、NGX_EVENT_MODULE等类型 */
...
typedef struct {
    ngx_str_t             name;
    void               *(*create_conf)(ngx_cycle_t *cycle); /* 解析配置文件前调用 */
    char               *(*init_conf)(ngx_cycle_t *cycle, void *conf); /* 解析配置文件后调用 */
} ngx_core_module_t;
