...
/* volatile: 随时从内存读取,不进行编译优化 */
volatile ngx_cycle_t  *ngx_cycle;
ngx_array_t            ngx_old_cycles; /* 动态数组，保存old_cycle，在ngx_init_cycle中初始化 */
...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->create_conf) {
			/* 即调用模块数组ngx_modules中核心模块的ngx_xxx_create_conf */
	...
	/* 解析配置文件 
	 * 一行行读取，对于遇到的指令,执行对应的ngx_command_t对象中定义的配置项处理方法 */
    ...
    for (i = 0; cycle->modules[i]; i++) {
        ...
        if (module->init_conf) {
			/* 即调用模块数组ngx_modules中核心模块的ngx_xxx_init_conf */
    ...
    if (ngx_test_config) {
		/* pid文件路径在ngx_core_module_init_conf初始化 */
